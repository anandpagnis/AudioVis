...
**Now centralized:** camera moves (CameraDirector), animation primitives (AnimationDirector), and post FX (EffectsDirector) all execute from `performanceState`, including the *choice* of camera mode — `pickCameraMode()` selects from the modes a scene declares, by mood and dramatic tension, re-taken at section boundaries.
