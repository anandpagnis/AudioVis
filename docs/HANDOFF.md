...
- A fresh checkout has no weights → `f.moodsValid` is `false` and every consumer
  (`MoodEstimator.partyBonus`, the new `danceBonus` fallback (F168), `AutoPilot`,
...
(Audit item 13 / F173. The hosting decision — how a *permissively-licensed* replacement would
