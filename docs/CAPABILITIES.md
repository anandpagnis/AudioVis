...
- **The post chain has never been measured.** It runs in every frame and its
  budget reservation (2 units) is a reasoned estimate, not data. `/bench`
  deliberately excludes post so scene costs compare cleanly — which leaves the
  one constant cost unmeasured. (F44)
