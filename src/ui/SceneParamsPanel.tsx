...
export function SceneParamsPanel() {
  ...
  const { params: declared, modes, paramLabels } = scene.metadata
          ...
          <div className="quality-row">
            {modes.map((m) => (
              <button
                ...
                onClick={() => useStore.getState().setSceneParams(target.id, { mode: m })}
              >
                {m}
              </button>
            ))}
      ...
      {rows.map(({ key, label }) => (
        <label key={key} className="param-row">
          ...
          <input
            ...
            value={resolved[key]}
            onChange={(e) =>
              useStore.getState().setSceneParams(target.id, {
                [key]: Number(e.target.value),
              } as Partial<Record<SceneParamKey, number>>)
            }
            ...
            onDoubleClick={() =>
              useStore.getState().setSceneParams(target.id, {
                [key]: declared?.[key] ?? DEFAULT_PARAM_VALUE,
              } as Partial<Record<SceneParamKey, number>>)
            }
          />
      ...
      {overridden && (
        <div className="param-row">
          ...
          <button
            ...
            onClick={() => useStore.getState().setSceneParams(target.id, null)}
          >
            reset {scene.name}
          </button>
        </div>
