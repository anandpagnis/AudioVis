...
export function useSceneParams(sceneId: string): Readonly<ResolvedSceneParams> {
  const spec = useMemo<SceneModeSpec | undefined>(() => {
    const meta = getScene(sceneId).metadata
    return meta.modes ? { modes: meta.modes, paramLabels: meta.paramLabels } : undefined
  }, [sceneId])

  const declared = useMemo(() => getScene(sceneId).metadata.params, [sceneId])
