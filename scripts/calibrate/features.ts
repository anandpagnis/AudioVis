...
/**
 * Dev A/B knob for the broadband energy term (F169). `loudness` runs the blend
...
export interface FrameSample {
  t: number
  rms: number
  /** BS.1770 K-weighted momentary loudness, BandNormalized (F169). */
  loudness: number
  energy: number
  sub: number
  bass: number
...
export function runTrack(
  ...
  // for AudioEngine's `audiovis-loudness` worklet (F156 Part A / F169). Fed
  ...
  for (let i = 0; i < totalFrames; i++) {
    ...
    // F169: the broadband energy term is still `f.rms`. Flipping it to
